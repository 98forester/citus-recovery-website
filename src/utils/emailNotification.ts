...
// Uses EmailJS for lead alerts and auto-replies.
// ──────────────────────────────────────────────
...
// ── User Auto-Reply (template_f69k4vn) ──────────────────────────────────
export const sendAutoReply = async (email: string, name: string): Promise<boolean> => {
    try {
        const response = await emailjs.send(
            import.meta.env.VITE_EMAILJS_SERVICE_ID,
            'template_f69k4vn',
            {
                to_name: name,
                to_email: email,
                reply_to: 'support@citusrecoverysolutions.com',
            },
            import.meta.env.VITE_EMAILJS_PUBLIC_KEY
        );
